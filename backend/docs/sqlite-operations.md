...
4. From the new release, run `uv run python main.py migrate` exactly once as the service account.
5. Run integrity/foreign-key checks, confirm `alembic current`, start one application process, and
   require `/health/ready` plus a representative authenticated read before reopening traffic. Use
   `python main.py serve --no-migrate` after this explicit migration; ordinary `serve` retains a
   convenient migrate-before-start default.
...
The `backup` command intentionally never runs migrations and has no `--no-migrate` option. Run it
with the currently deployed code before installing or invoking migrations so the backup preserves
the current schema and can be paired with the previous application release. The in-memory `batch`
command likewise never accesses or migrates the database. Never substitute `cp pm_sim.db` while the
service is running.
...
5. With traffic still stopped, run `uv run alembic current`, deploy compatible code, and run
   `uv run python main.py migrate` only if the restore plan calls for upgrading the restored copy.
   To roll back an incompatible migration, instead restore the pre-migration backup and previous
   application release; do not immediately re-upgrade it at startup.
