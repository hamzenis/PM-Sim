...
| `app/batch` | In-memory repeated deterministic execution, strategies, summaries, CSV/JSON reports. `run_batch`, `run_simulation`, `run_simulation_batch`, `report_to_dict`, `report_to_csv`. | Scenario model/adapter and pure simulation package. | None. | Database/session, FastAPI, persisted runs, authored-content delivery state. |
