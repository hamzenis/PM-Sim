...
class TemplateScenarioSerializer(serializers.ModelSerializer):
    management_goal = ManagementGoalSerializer(required=False)
    question_collections = QuestionCollectionSerializer(many=True, required=False)
    simulation_fragments = SimulationFragmentSerializer(many=True, required=False)
    score_card = ScoreCardSerializer(required=False)
    ...
    def _create_management_goal(self, validated_data, template_scenario):
        if "management_goal" in validated_data:
            management_goal_data = validated_data.pop("management_goal", {}) or {}
            ManagementGoal.objects.create(
                template_scenario=template_scenario, **management_goal_data
            )

    def _create_question_collection(self, validated_data, template_scenario):
        if "question_collections" in validated_data:
            question_collection_data = (
                validated_data.pop("question_collections", []) or []
            )
    ...
    def _create_simulation_fragment(self, validated_data, template_scenario):
        if "simulation_fragments" in validated_data:
            simulation_fragments_data = (
                validated_data.pop("simulation_fragments", []) or []
            )
    ...
    def _create_score_card(self, validated_data, template_scenario):
        if "score_card" in validated_data:
            score_card_data = validated_data.pop("score_card", {}) or {}
            ScoreCard.objects.create(
                template_scenario=template_scenario, **score_card_data
            )

    def _create_event(self, validated_data, template_scenario):
        if "events" in validated_data:
            events_data = validated_data.pop("events", []) or []
    ...
    def create(self, validated_data, _id=None):
        ...
        self._create_management_goal(validated_data, template_scenario)

        # 2. create question collection
        self._create_question_collection(validated_data, template_scenario)

        # 3 create simulation_fragments
        self._create_simulation_fragment(validated_data, template_scenario)

        # 4. create score card
        self._create_score_card(validated_data, template_scenario)

        # 5. create event
        self._create_event(validated_data, template_scenario)
