...
class StartUserScenarioView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            msg = f"'{e.__class__.__name__}' occurred when creating user scenario: {str(e)}"
            logging.error(msg)
            logging.debug(e)
            return Response(
    ...
    def post(self, request):
        ...
        if request.data.get("type") is None:
            return Response(
                {
                    "status": "error",
                    "error-message": "Type of request was not specified. Type has to be one of the following: QUESTION, SIMULATION",
                },
