...
def simulate(request, session: CachedScenario) -> None:
    """
    This function does the actual simulation of a scenario fragment.

    :param request: Object with request data, like a SimulationRequest
    :param session: The CachedScenario object representing the current simulation session
    """
    if request.actions is None:
        raise RequestActionException()

    normal_work_hour_day: int = 8
    workpack = request.actions
    logging.info(f"Workpack: {workpack}")  # DEBUG: Log the received workpack
    days = workpack.days
    ...
    if request.members and request.members != []:
        member_change = request.members
    ...
    if request.actions.teamevent:
        days = days - 1

    # integration test: Will be at the end of the week
    if request.actions.integrationtest:
        days = days - 1
    ...
    if request.actions.integrationtest:
        ...
        session.scenario.state.day += 1
        session.scenario.state.cost += sum(
            member.skill_type.cost_per_day for member in session.members
        )
        for t in tasks_to_integration_test:
            if t.correct_specification:
                ...
                # Stress increase for all members, distributed equally among them
                str_incr = session.scenario.config.stress_error_increase / len(
                    session.members
                )
                for member in session.members:
                    member.stress = min(member.stress + str_incr, 1)
...
def continue_simulation(session: CachedScenario, request) -> ScenarioResponse:
    """
    Continues the simulation based on the request and the current scenario state.

    :param session: The CachedScenario object representing the current simulation session
    :type session: CachedScenario
    ...
    start = time.perf_counter()
    request_handling_mapper[request.type](request, session)
    logging.warning(f"{time.perf_counter() - start} seconds.")
    ...
    if isinstance(event, Event):
        ...
        return complete_scenario_step(session, request, scenario_response)
    ...
    # 5.3 Check if next component is a Result -> Scenario is finished
...
def complete_scenario_step(session: CachedScenario, req, scenario_response):
    write_history(session.scenario, req, scenario_response.type) # DEBUG: History writing
