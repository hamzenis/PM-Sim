...
class CachedTasks:
    ...
    def len_tasks(self) -> int:
        """Returns the total number of tasks in this cached set."""
        return len(self.tasks)
    
    def len_tasks_worked_on(self) -> int:
        """
        Returns the number of tasks that have been worked on.
        Defined as tasks that are done, but does not mean they are
        integration tested, has correct specification, unit tested, or bug free.
        Only if a task is only worked on(done=True) it counts here.
        """
        return len([t for t in self.tasks if t.done ])
