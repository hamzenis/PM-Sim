...
vi.mock('../api/scenarios', () => ({ archiveScenario: vi.fn(), createScenario: vi.fn(), listAvailableScenarios: vi.fn(), listOwnedScenarios: vi.fn(), publishScenarioRevision: vi.fn(), validateScenario: vi.fn() }));
...
const scenario = { id: revisionId, class_id: classId, class_name: 'Project Management 101', revision_number: 2, definition: { name: 'New Product Launch', description: 'Guide a team through a high-stakes product launch.' } };

const renderStudentOverview = () => render(
	<ChakraProvider><AuthContext.Provider value={{ currentUser: { role: 'student' } }}><MemoryRouter initialEntries={['/scenarios']}><Routes><Route path="/scenarios" element={<ScenarioOverview />} /><Route path="/simulations/:run_id" element={<div>Simulation destination</div>} /></Routes></MemoryRouter></AuthContext.Provider></ChakraProvider>
);

beforeEach(() => { vi.clearAllMocks(); listAvailableScenarios.mockResolvedValue([scenario]); listSimulationRuns.mockResolvedValue([]); });
...
test('shows a helpful empty state when no scenarios are assigned', async () => {
	listAvailableScenarios.mockResolvedValue([]); renderStudentOverview();
	expect(await screen.findByRole('heading', { name: 'No scenarios assigned yet' })).toBeInTheDocument();
	expect(screen.getByText(/professor makes them available/i)).toBeInTheDocument();
});
...
test('shows week progress and continues an active run', async () => {
	listSimulationRuns.mockResolvedValue([{ id: runId, class_id: classId, scenario_revision_id: revisionId, status: 'active', current_week: 3 }]); renderStudentOverview();
...
test('shows a completed run with a result action', async () => {
	listSimulationRuns.mockResolvedValue([{ id: runId, class_id: classId, scenario_revision_id: revisionId, status: 'deadline_reached', current_week: 8 }]); renderStudentOverview();
...
test('shows a clear error and permits retry when starting fails', async () => {
	startSimulationRun.mockRejectedValueOnce(new Error('network unavailable')).mockResolvedValueOnce({ id: runId }); renderStudentOverview();
...
test('does not expose UUIDs and prevents duplicate starts while a request is active', async () => {
	let resolveStart; startSimulationRun.mockReturnValue(new Promise((resolve) => { resolveStart = resolve; }));
	const { container } = renderStudentOverview();
	const startButton = await screen.findByRole('button', { name: 'Start' }); fireEvent.click(startButton); fireEvent.click(startButton);
	...
	resolveStart({ id: runId }); await screen.findByText('Simulation destination');
});
