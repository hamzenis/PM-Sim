...
const TaskLineChart = ({ title, data }) => {
  ...
  const [tasksUnitTested, setTasksUnitTested] = useState(
    data.tasks.tasks_unit_tested
  );
  const [tasksUnitTestedBefore, setTasksUnitTestedBefore] = useState(0);

  const [tasksIntegrationTested, setTasksIntegrationTested] = useState(
    data.tasks.tasks_integration_tested
  );
  const [tasksIntegrationTestedBefore, setTasksIntegrationTestedBefore] =
    useState(0);
    ...
    >
      ...
      <VStack minW="200px" alignItems="baseline">
        ...
        <Stat>
          ...
          <StatHelpText>
            <StatArrow type="increase" />
            {tasksIntegrationTested - tasksIntegrationTestedBefore} since last
            iteration
          </StatHelpText>
        </Stat>
        <Stat>
        ...
        <Stat>
          ...
          <StatNumber>
            {firstIteration && tasksBug === 0 ? "?" : tasksBug}
          </StatNumber>
          <StatHelpText>
            <StatArrow type="increase" />
